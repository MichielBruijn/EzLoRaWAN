...
u16_t os_rlsbf2 (const u8_t* buf) {
    return (u16_t)((u16_t)buf[0] | ((u16_t)buf[1]<<8));
}
#endif

#if !defined(os_rmsbf2)
u16_t os_rmsbf2 (const u8_t* buf) {
    return (u16_t)((buf[0]<<8) | buf[1]);
}
#endif

#if !defined(os_rlsbf4)
u32_t os_rlsbf4 (const u8_t* buf) {
    return (u32_t)((u32_t)buf[0] | ((u32_t)buf[1]<<8) | ((u32_t)buf[2]<<16) | ((u32_t)buf[3]<<24));
}
#endif


#if !defined(os_rmsbf4)
u32_t os_rmsbf4 (const u8_t* buf) {
    return (u32_t)((u32_t)buf[3] | ((u32_t)buf[2]<<8) | ((u32_t)buf[1]<<16) | ((u32_t)buf[0]<<24));
}
#endif


#if !defined(os_wlsbf2)
void os_wlsbf2 (u8_t* buf, u16_t v) {
    buf[0] = v;
    buf[1] = v>>8;
}
#endif
...
void os_wmsbf2 (u8_t* buf, u16_t v) {
    buf[0] = v>>8;
    buf[1] = v;
}
#endif
...
void os_wlsbf3 (u8_t* buf, u32_t v) {
    buf[0] = v;
    buf[1] = v>>8;
    buf[2] = v>>16;
}
...
void os_wlsbf4 (u8_t* buf, u32_t v) {
    buf[0] = v;
    buf[1] = v>>8;
    buf[2] = v>>16;
    buf[3] = v>>24;
...
void os_wmsbf4 (u8_t* buf, u32_t v) {
    buf[3] = v;
    buf[2] = v>>8;
    buf[1] = v>>16;
    buf[0] = v>>24;
...
u16_t os_crc16 (u8_t* data, uint len) {
    u16_t remainder = 0;
    u16_t polynomial = 0x1021;
    for( uint i = 0; i < len; i++ ) {
        remainder ^= data[i] << 8;
        for( u8_t bit = 8; bit > 0; bit--) {
            if( (remainder & 0x8000) )
...
static const u8_t DR2RPS_EU[16] = {
...
static const u8_t DR2RPS_125kHz[16] = {
    LORA_UP_RPS(SF12, BW125),    LORA_UP_RPS(SF11, BW125),    LORA_UP_RPS(SF10, BW125),    LORA_UP_RPS(SF9,  BW125),
...
static const u8_t DR2RPS_US[16] = {
...
static const u8_t DR2RPS_AU[16] = {
    LORA_UP_RPS(SF12, BW125),    LORA_UP_RPS(SF11, BW125),    LORA_UP_RPS(SF10, BW125),    LORA_UP_RPS(SF9,  BW125),
...
static const u8_t DR2RPS_IN[16] = {
    LORA_UP_RPS(SF12, BW125),    LORA_UP_RPS(SF11, BW125),    LORA_UP_RPS(SF10, BW125),    LORA_UP_RPS(SF9,  BW125),
...
static const LMIC_REGION_t REGIONS[REGIONS_COUNT] = {
#ifdef CFG_eu868
    [LMIC_REGION_EU868] = {
...
#define isREGION(reg)   (&REGION == &REGIONS[LMIC_REGION_##reg])
...
static dr_t lowerDR (dr_t dr, u8_t n) {
    if (dr == CUSTOM_DR)
        return dr;
    return dr <= n ? 0 : dr-n;
}
...
static const u8_t SENSITIVITY[7][3] = {
    // ------------bw----------
...
ostime_t calcAirTime (rps_t rps, u8_t plen) {
    if( isFsk(rps) ) {
        return (plen+/*preamble*/5+/*syncword*/3+/*len*/1+/*crc*/2) * /*bits/byte*/8
            * (s32_t)OSTICKS_PER_SEC / /*kbit/s*/50000;
    }
    u8_t bw = getBw(rps);  // 0,1,2 = 125,250,500kHz
    u8_t sf = getSf(rps);
    u8_t sfx = 4*(sf+(7-SF7));
    u8_t q = sfx - 8*enDro(rps);
...
//
static ostime_t dr2hsym (dr_t dr, s8_t num) {
    rps_t rps = updr2rps(dr);
    u8_t sf = getSf(rps);
    u8_t bw = getBw(rps);
    LMIC_STATIC_ASSERT(BW125 == 0, "BW125 assumed to be 0");
    ASSERT(sf >= SF7 && sf <= SF12 && bw <= BW500);
    s32_t us = num << (9 + sf - SF7 - bw);  // 10 => 9 half symbol time
    return us2osticks(us);
}

#if !defined(DISABLE_CLASSB)
static ostime_t calcRxWindow (u8_t secs, dr_t dr) {
    ostime_t rxoff, err;
...
static void iniRxdErr () {
    ...
    // Min(rxdErrs) == +RXDERR_INI/2
    LMIC.rxdErrs[0] = 0;
    for( u8_t u=RXDERR_NUM&0; u<RXDERR_NUM; u++ )
        LMIC.rxdErrs[u] = (us2osticksCeil(RXDERR_INI) << (RXDERR_SHIFT-1)) * (u&1?-1:1);
    LMIC.rxdErrIdx = 0;
}

static void addRxdErr (u8_t rxdelay) {
    s32_t err = (((LMIC.rxtime0 - LMIC.txend) - sec2osticks(rxdelay)) << RXDERR_SHIFT) / rxdelay;
    if( (u32_t)((err>>20)+1) > 1 )  // overflow?
        return;
...
static s32_t evalRxdErr (u32_t* span) {
    s32_t min = 0x7FFFFFFF, min2=0x7FFFFFFF;
    s32_t max = 0x80000000, max2=0x80000000;
    s32_t sum = 0;
    for( u8_t u=0; u<RXDERR_NUM; u++ ) {
        s32_t v = LMIC.rxdErrs[u];
...
static void adjustByRxdErr (u8_t rxdelay, u8_t dr) {
#ifdef CFG_testpin
    // XXX: for now only in HW regr tests - not yet ready for prime time
    u32_t span;
    s32_t skew = evalRxdErr(&span);
...
static bit_t rxschedNext (rxsched_t* rxsched, ostime_t cando) {
  again:
    ...
    if( (slot=rxsched->slot) >= 128 )
        return 0;
    u8_t intv = 1<<(rxsched->intvExp & 0x7);
...
static ostime_t rndDelay (u8_t secSpan) {
    u16_t r = os_getRndU2();
    ostime_t delay = r;
    if( delay > OSTICKS_PER_SEC )
        delay = r % (u16_t)OSTICKS_PER_SEC;
    if( secSpan > 0 )
        delay += ((u8_t)r % secSpan) * OSTICKS_PER_SEC;
    return delay;
}


static void txDelay (ostime_t reftime, u8_t secSpan) {
...
static void setDrJoin (u8_t reason, dr_t dr) {
    (void)reason; // unused
    LMIC.datarate = dr;
}


static void setDrTxpow (u8_t reason, dr_t dr, s8_t powadj) {
    (void)reason; // unused
...
u8_t LMIC_setPingable (u8_t intvExp) {
    ASSERT(intvExp <= 7);
    // Change setting
...
static dr_t prepareDnDr (dr_t updr) {
    ...
    s8_t dndr = (s8_t) updr + REGION.rx1DrOff[LMIC.dn1DrOffIdx];
    dr_t dr8 = REGION.dr2rps[8];
    s8_t mindr = (dr8 != ILLEGAL_RPS && getNocrc(dr8)) ? 8 : 0;
    if( dndr < mindr )
        dndr = mindr;
    else if( dndr >= 16 )
        dndr = 15;
...
static void disableChannel_dyn (u8_t chidx) {
...
static bit_t setupChannel_dyn (u8_t chidx, freq_t freq, drmap_t drmap) {
...
static void initDefaultChannels_dyn (void) {
    ...
    for (u8_t ch = 0; ch < MIN_DYN_CHNLS && REGION.defaultCh[ch]; ch++) {
        setupChannel_dyn(ch, REGION.defaultCh[ch], defaultDrMap);
    }
}

static u8_t applyChannelMap_dyn (u8_t chpage, u16_t chmap, u16_t* dest) {
    if( chpage == MCMD_LADR_CHP_ALLON ) {
        chmap = 0;
        for( u8_t ci=0; ci<MAX_DYN_CHNLS; ci++ ) {
...
static u8_t checkChannelMap_dyn (u16_t* map) {
    if( *map == 0) {
...
static u8_t selectRandomChnl (u16_t map, u8_t nbits) {
    u8_t k;
 again:
    // Note: we have a small negligible bias of 2^16 % nbits (nbits <= 16 => bias < 0.025%)
    k = os_getRndU2() % nbits;
    for( u8_t chnl=0; chnl<16; chnl++ ) {
        if( (map & (1<<chnl)) == 0 )
            continue;
        if( k==0 ) {
...
static ostime_t nextTx_dyn (ostime_t now) {
    ...
    u8_t cccnt = 0; // number of candidate channels
    u16_t ccmap = 0; // candidate channel mask
    u16_t pcmap = 0; // probe channel mask
again:
    for (u8_t chnl = 0; chnl < MAX_DYN_CHNLS; chnl++) {
        u16_t chnlbit = 1 << chnl;
    ...
    if (cccnt) {
        debug_verbose_printf("%u channels are available now\r\n", cccnt);
        while( cccnt ) {
            u8_t chnl = selectRandomChnl(ccmap, cccnt);
...
static void perm (unsigned char* p, int start, int end, u8_t* prng) {
...
static void generateHopList (u8_t* hoplist, int nch) {
    int nb = nch >> 3;  // number of 8-ch blocks

    u8_t prng[16];      // prng state
    memcpy(prng, "\x10hoplist", 8);
    os_getDevEui(prng + 8);

    u8_t bp[nb]; // block permutation
...
static u8_t enableAllChannels_fix (void) {
    int nch = numChannels();
    u8_t rv = 0;
    for (u8_t i = 0; i < (nch >> 4); i++) {
...
static void disableChannel_fix (u8_t chidx) {
    ...
    for (u8_t i = 0; i < sizeof(LMIC.fix.channelMap) / sizeof(u16_t); i++) {
...
static int activeFhssChannelCount_fix (u16_t* channelMap) {
    int i, cc = 0;
    for( i = 0; i < REGION.numChBlocks; i+=2 ) {
        u16_t map = channelMap[i/2];
...
static u8_t checkChannelMap_fix (u16_t* map) {
#if CFG_us915
...
#endif
    u16_t anyon = 0;
    for( u8_t i=0; i<CHMAP_SZ; i++ ) {
        anyon |= map[i];
    }
    return !!anyon;
}

static u8_t applyChannelMap_fix (u8_t chpage, u16_t chmap, u16_t* dest) {
    if (chpage == MCMD_LADR_CHP_125ON || chpage == MCMD_LADR_CHP_125OFF) {
        u16_t en125 = (chpage == MCMD_LADR_CHP_125ON) ? 0xFFFF : 0x0000;
        for (u8_t u = 0; u < (REGION.numChBlocks >> 1); u++) {
            dest[u] = en125;
        }
...
static void updateTx_fix (ostime_t txbeg) {  //XXX:BUG: this is US915/AU915 centric - won't work for CN470
  ...
  updateGDC:
    ...
    debug_verbose_printf("Updating info for TX at %t, airtime will be %t, frequency %.2F.\r\n", txbeg, airtime, LMIC.freq, 6);
    if( LMIC.globalDutyRate != 0 )
        debug_verbose_printf("Updating global duty avail to %t\r\n", LMIC.globalDutyAvail);
}

// check if a channel is available in the map that supports this datarate
static bool checkChannel_fix (u16_t* map, dr_t dr) {
    u8_t beg8, end8;
    if( dr == REGION.fixDr ) {
...
static ostime_t nextTx_fix (ostime_t now) {
    if( LMIC.opmode & OP_NEXTCHNL ) {
        if( LMIC.datarate == REGION.fixDr ) {
            u8_t off = REGION.numChBlocks*8;
            for( u8_t i=0, e=REGION.numChBlocks; i<e; i++ ) {  // assuming for every 8ch 125 is one fix DR ch (iff fixDr != ILLEGAL_DR)
                u8_t chnl = ++LMIC.refChnl % e + off;
            ...
            // 125kHz
            for( u8_t i=0, e=REGION.numChBlocks*8; i<e; i++ ) {
                u8_t chnl = LMIC.fix.hoplist[++LMIC.refChnl % e];
                if( (LMIC.fix.channelMap[(chnl >> 4)] & (1<<(chnl & 0xF))) != 0 ) {
                    LMIC.txChnl = chnl;
                    break;
                }
...
static int decodeBeacon (void) {
    ...
    LMIC.bcninfo.lat    = (s32_t)os_rlsbf4(&d[REGION.beaconOffInfo]) >> 8;   // read as signed 24-bit
...
static bit_t decodeFrame (void) {
    ...
    u32_t addr  = os_rlsbf4(&d[OFF_DAT_ADDR]);
    u32_t seqno = os_rlsbf2(&d[OFF_DAT_SEQNO]);
...
#endif
    seqno = *pseqnoDn + (s16_t)(seqno - *pseqnoDn);

    if( !lce_verifyMic(LCE_NWKSKEY, LMIC.devaddr, seqno, d, pend) ) {
        goto norx;
    }
    if( seqno < *pseqnoDn ) {
        if( (s32_t)seqno > (s32_t)*pseqnoDn ) {
            goto norx;
        }
    ...
    while( oidx < olen ) {
        ...
        case MCMD_LADR_REQ: {
            ...
            u8_t ans = MCMD_LADR_ANS_POWACK | MCMD_LADR_ANS_CHACK | MCMD_LADR_ANS_DRACK;
            do {
            ...
            s8_t powadj = (s8_t)((p1 & MCMD_LADR_POW_MASK) >> MCMD_LADR_POW_SHIFT);
#if 0
        ...
        case MCMD_DN2P_SET: {
            u8_t dr = opts[oidx+1] & 0xF;
            u8_t off = (opts[oidx+1] >> 4) & 7;
        ...
        case MCMD_SNCH_REQ: {
            u8_t ans = MCMD_SNCH_ANS_PEND;
#ifdef REG_DYN
            if (!REG_IS_FIX()) { // only available in dynamic regions
                u8_t chidx = opts[oidx+1];
                freq_t freq  = rdFreq(&opts[oidx+2]);
                if( chidx < MIN_DYN_CHNLS && REGION.defaultCh[chidx] ) {
                    ...
                    u8_t mindr = opts[oidx+5] & 0xF;
                    u8_t maxdr = opts[oidx+5] >> 4;
        ...
        case MCMD_TIME_ANS: {
            u32_t secs = os_rlsbf4(&opts[oidx+1]);
            u8_t frac = opts[oidx+5];
...
static bit_t decodeMultiCastFrame (void) {
    ...
    seqno = s->seqnoADn + (u16_t)(seqno - s->seqnoADn);
...
static void schedRx2 (u8_t delay, osjobcb_t func) {
...
static void txDone (u8_t delay, osjobcb_t func) {
#if !defined(DISABLE_CLASSB)
...
static bit_t processJoinAccept (void) {
    ...
    u8_t hdr  = LMIC.frame[0];
    u8_t dlen = LMIC.dataLen;
    ...
    if( dlen > LEN_JA ) {
        ...
        if (REG_IS_FIX()) {
            ...
            for (u8_t i=0; i < 8 && i < CHMAP_SZ; i++, dlen += 2) {
...
#endif // REG_FIX
        {
            ...
            for( u8_t chidx=3; chidx<8; chidx++, dlen+=3 ) {
                s32_t freq = rdFreq(&LMIC.frame[dlen]);
                if( freq > 0 ) {
...
static void buildDataFrame (void) {
    ...
    u8_t dlen = txdata ? LMIC.pendTxLen : 0;
    ...
    if( LMIC.dnfqAns || LMIC.dnfqAnsPend ) {
        // Note: this is cleared with reception of a frame in a class A RX1/RX2 window
        for( u8_t i=0; i < LMIC.dnfqAnsPend && end <= OFF_DAT_OPTS + 15 - 2; i++ ) {
...
u8_t LMIC_enableTracking (u8_t tryAskForTime) {
...
static void buildJoinRequest (u8_t ftype) {
...
static bit_t processDnData (void) {
    ...
    if( LMIC.dataLen == 0 ) {
      ...
      txcomplete:
        ...
        if( LMIC.adrAckReq >= (s32_t) LINK_CHECK_DEAD ) {
...
void LMIC_setDrTxpow (dr_t dr, s8_t txpowadj) {
...
void LMIC_selectChannel(u8_t channel) {
    LMIC.opmode &= ~OP_NEXTCHNL;
    LMIC.txChnl = channel;
...
int LMIC_regionIdx (u8_t regionCode) {
...
u8_t LMIC_regionCode (u8_t regionIdx) {
    ASSERT(regionIdx < REGIONS_COUNT);
    return REGIONS[regionIdx].regcode;
}

void LMIC_reset_ex (u8_t regionCode) {
    os_radio(RADIO_STOP);
    os_clearCallback(&LMIC.osjob);

    os_clearMem((u8_t*) &LMIC, sizeof(LMIC));
...
//
int LMIC_setTxData2 (u8_t port, u8_t* data, u8_t dlen, u8_t confirmed) {
...
void LMIC_setSession (u32_t netid, devaddr_t devaddr, const u8_t* nwkKey,
#if defined(CFG_lorawan11)
        const u8_t* nwkKeyDn,
#endif
        const u8_t* appKey) {
...
int LMIC_setMultiCastSession (devaddr_t grpaddr, const u8_t* nwkKeyDn, const u8_t* appKey, u32_t seqnoADn) {
    session_t* s;
...
void LMIC_setLinkCheck (u32_t limit, u32_t delay) {
    LMIC.adrAckLimit = limit;
    LMIC.adrAckDelay = delay;
    LMIC.adrAckReq = LINK_CHECK_INIT;
}

bit_t LMIC_setupChannel (u8_t chidx, freq_t freq, u16_t drmap) {
    if (REG_IS_FIX()) {
...
void LMIC_disableChannel (u8_t chidx) {
...
rps_t LMIC_updr2rps (u8_t dr) {
    return updr2rps(dr);
}

rps_t LMIC_dndr2rps (u8_t dr) {
    return dndr2rps(dr);
}

ostime_t LMIC_calcAirTime (rps_t rps, u8_t plen) {
    return calcAirTime(rps, plen);
}

u8_t LMIC_maxAppPayload () {
...
ostime_t LMIC_dr2hsym (dr_t dr, s8_t num) {
...
void LMIC_getRxdErrInfo (s32_t* skew, u32_t* span) {
