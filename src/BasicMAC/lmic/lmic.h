...
typedef struct {
    void     (*disableChannel) (u8_t chidx);
    void     (*initDefaultChannels) (void);
    void     (*prepareDn) ();
    u8_t     (*applyChannelMap) (u8_t chpage, u16_t chmap, u16_t* dest);
    u8_t     (*checkChannelMap) (u16_t* map);
...
typedef struct {
    ...
    union {
        ...
        struct {
            ...
            u16_t        chTxCap;                        // per-channel DC
            u16_t        ccaTime;                        // CCA time (ticks)
            s8_t        ccaThreshold;                   // CCA threshold
        ...
        struct {
            ...
            u8_t        numChBlocks;                    // number of 8-channel blocks
            u8_t        numChDnBlocks;                  // number of 8-channel blocks for downlink
    ...
    const u8_t*         dr2rps;                 // pointer to DR table
    ...
    u8_t                beaconOffInfo;          // offset beacon info field
    u8_t                beaconLen;              // beacon length
    ostime_t            beaconAirtime;          // beacon air time
    eirp_t              maxEirp;                // max. EIRP (initial value)
    s8_t                rx1DrOff[8];            // RX1 data rate offsets
    u8_t                dr2maxAppPload[16];     // max application payload (assuming no repeater and no fopts)
    u8_t                regcode;                // external region code
...
#define LINK_CHECK_OFF  ((s32_t)0x80000000)
#define LINK_CHECK_INIT ((s32_t)(-LMIC.adrAckLimit))
...
typedef struct {
    u8_t     dr;
    u8_t     intvExp;   // bits: 7:pend, 3:illegal intv, 2-0:intv
    u8_t     slot;      // runs from 0 to 128
    u8_t     rxsyms;
    ostime_t rxbase;
...
typedef struct {
    ...
    s8_t     rssi;    //!< Adjusted RSSI value of last received beacon
    s8_t     snr;     //!< Scaled SNR value of last received beacon
    u8_t     flags;   //!< Last beacon reception and tracking states. See BCN_* values.
    u32_t     time;    //!< GPS time in seconds of last beacon (received or surrogate)
    //
    u8_t     info;    //!< Info field of last beacon (valid only if BCN_FULL set)
    s32_t     lat;     //!< Lat field of last beacon (valid only if BCN_FULL set)
    s32_t     lon;     //!< Lon field of last beacon (valid only if BCN_FULL set)
} bcninfo_t;
#endif
...
typedef struct {
    ...
    u8_t        nwkKeyDn[16]; // network session key for down-link
    u8_t        appKey[16];   // application session key
    u32_t        seqnoADn;     // down stream seqno (AFCntDown)
} session_t;

// duty cycle/dwell time relative to baseAvail in sec.
// To avoid roll over this needs to be updated.
typedef u16_t avail_t;
...
struct lmic_t {
    ...
    u8_t        rxsyms;
    u8_t        dndr;
    s8_t        txpow;     // dBm -- needs to be combined with brdTxPowOff

    osjob_t     osjob;

    osxtime_t   baseAvail;                      // base time for availability
    avail_t     globalAvail;                    // next available DC (global)
    u8_t        noDC;                           // disable all duty cycle

    const LMIC_REGION_t* region;
    union {
#ifdef REG_DYN
        ...
        struct {
            ...
            u16_t        channelMap;             // active channels
        } dyn;
#endif
#ifdef REG_FIX
        // FCC-like (fixed channels)
        struct {
            u16_t        channelMap[CHMAP_SZ];           // enabled bits
            u8_t        hoplist[MAX_FIX_CHNLS_125];     // hoplist
        } fix;
#endif
    };

    u8_t        refChnl;         // channel randomizer - search relative to this indicator
    u8_t        txChnl;          // channel for next TX
    u8_t        globalDutyRate;  // max rate: 1/2^k
    ostime_t    globalDutyAvail; // time device can send again  -- XXX:PROBLEM if no TX for ~18h we have a rollover here!! --> avail_t??

    u32_t        netid;        // current network id (~0 - none)
    u16_t        opmode;
    u8_t        clmode;       // current/pending class A/B/C
    u8_t        pollcnt;      // >0 waiting for an answer from network
    u8_t        nbTrans;      // ADR controlled frame repetition
    s8_t        txPowAdj;     // adjustment for txpow (ADR controlled)
    s8_t        brdTxPowOff;  // board-specific power adjustment offset
    dr_t        datarate;     // current data rate
    cr_t        errcr;        // error coding rate (used for TX only)
    u8_t        rejoinCnt;    // adjustment for rejoin datarate
    s16_t        drift;        // last measured drift
    s16_t        lastDriftDiff;
    s16_t        maxDriftDiff;
    osxtime_t   gpsEpochOff;  // gpstime = gpsEpochOff+getXTime(), 0=undefined
    s32_t        rxdErrs[RXDERR_NUM];
    u8_t        rxdErrIdx;

    u8_t        pendTxPort;
    u8_t        pendTxConf;   // confirmed data
    u8_t        pendTxLen;    // +0x80 = confirmed
    u8_t        pendTxData[MAX_LEN_PAYLOAD];
    u8_t        pendTxNoRx;   // don't listen for down data after tx

    u16_t        devNonce;     // last generated nonce
    lce_ctx_t   lceCtx;
    devaddr_t   devaddr;
    u32_t        seqnoDn;      // device level down stream seqno
#if defined(CFG_lorawan11)
    u32_t        seqnoADn;     // device level down stream seqno (AFCntDown)
#endif
    u32_t        seqnoUp;

    u8_t        dnConf;       // dn frame confirm pending: LORA::FCT_ACK or 0
    s32_t        adrAckReq;    // counter until we reset data rate (0x80000000=off)
    u32_t        adrAckLimit;  // ADR_ACK_LIMIT
    u32_t        adrAckDelay;  // ADR_ACK_DELAY

    u8_t        margin;       // bits 7/6:RFU, 0-5: SNR of last DevStatusReq frame, reported by DevStatusAns to network
    u8_t        gwmargin;     // last reported by network via LinkCheckAns
    u8_t        gwcnt;        //  - ditto -
    u8_t        foptsUpLen;
    u8_t        foptsUp[64];  // pending FOpts in up direction - cleared after next send
    bit_t       devsAns;      // device status answer pending
    u8_t        adrEnabled;
    u8_t        moreData;     // NWK has more data pending
    bit_t       dutyCapAns;   // have to ACK duty cycle settings
    //XXX:old: u8_t        snchAns;      // answer set new channel
    u8_t        dn1Dly;       // delay in secs to DNW1
    s8_t        dn1DrOffIdx;  // index into DR offset table (can be negative in some regions!)
    // 2nd RX window (after up stream)
    u8_t        dn2Dr;
    u32_t        dn2Freq;
    u8_t        dn2Ans;       // 0=no answer pend, 0x80+ACKs
    u8_t        dn1DlyAns;    // 0=no answer pend, 0x80 send MCMD_RXTM_ANS
    u8_t        dnfqAns;      // # of DNFQ in this down frame
    u8_t        dnfqAnsPend;  // pending ACK bits (2 each)
    u32_t        dnfqAcks;     // ack bit pending

    // multicast sessions
    session_t  sessions[MAX_MULTICAST_SESSIONS];

#if defined(CFG_lorawan11)
    u8_t        opts;         // negotiated protocol options
#endif

#if !defined(DISABLE_CLASSB)
    // Class B state
    u8_t        missedBcns;   // unable to track last N beacons
    s8_t        askForTime;   // how often to ask for time
    //XXX:old: u8_t        pingSetAns;   // answer set cmd and ACK bits
    rxsched_t   ping;         // pingable setup
#endif

    // Public part of MAC state
    u8_t        txCnt;
    u8_t        txrxFlags;  // transaction flags (TX-RX combo)
    u8_t        dataBeg;    // 0 or start of data (dataBeg-1 is port)
    u8_t        dataLen;    // 0 no data or zero length data, >0 byte count of data
    u8_t        frame[MAX_LEN_FRAME];

#if !defined(DISABLE_CLASSB)
    u8_t        bcnfAns;      // mcmd beacon freq: bit7:pending, bit0:ACK/NACK
    u8_t        bcnChnl;
    u32_t        bcnFreq;      // 0=default, !=0: specific BCN freq/no hopping
    u8_t        bcnRxsyms;    //
    ostime_t    bcnRxtime;
    bcninfo_t   bcninfo;      // Last received beacon info
#endif

    u8_t        noRXIQinversion;
    ...
    u32_t        radioPwr_ua;  // power consumption of current radio operation in uA
...
bit_t LMIC_setupChannel (u8_t channel, freq_t freq, u16_t drmap);
void  LMIC_disableChannel (u8_t channel);
...
void  LMIC_setDrTxpow   (dr_t dr, s8_t txpow);  // set default/start DR/txpow
...
void  LMIC_reset_ex     (u8_t regionIdx);
...
#if !defined(DISABLE_CLASSB)
u8_t  LMIC_enableTracking  (u8_t tryBcnInfo);
